  ...
  return (
    <Link
      ...
      <img
        ...
        alt={friend.name}
      />
      <h3 className="text-xl font-bold text-gray-800 group-hover:text-[#EFAD44] transition-colors">
        {friend.name}
      </h3>
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-4">
        {friend.tags.map((tag) => (
          <span
            key={tag}
            className="bg-green-50 text-green-700 text-xs px-4 py-1 rounded-2xl uppercase font-bold tracking-wider">
            {tag}
          </span>
        ))}
      </div>
      <div
        className={`py-2 px-4 rounded-full text-xs font-bold capitalize ${statusColors[friend.status]} transition-colors`}>
        {friend.status}
      </div>
    </Link>
  );
