    ...
    <div className="max-w-6xl mx-auto px-4 py-8 md:py-12 grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-1 space-y-4">
        <div className="bg-white p-6 md:p-8 rounded-xl border border-gray-100 shadow-sm text-center cursor-pointer group transition-all">
          <img
            src={friend.picture}
            className="w-32 h-32 rounded-full mx-auto border-4 border-gray-50 object-cover mb-2"
            alt={friend.name}
          />
          <h2 className="text-2xl font-bold text-gray-800 group-hover:text-[#EFAD44] transition-colors">
            {friend.name}
          </h2>

          <div
            className={`badge my-2 py-4 px-5 uppercase rounded-full text-[11px] font-bold border-none text-white ${friend.status === 'overdue' ? 'bg-red-500' : friend.status === 'almost due' ? 'bg-orange-400' : 'bg-[#244D3F]'}`}>
            {friend.status}
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-2 my-2">
            {friend.tags.map((tag) => (
              <span
                key={tag}
                className="bg-green-50 text-green-700 text-xs uppercase px-3 py-1 rounded-2xl font-bold">
                # {tag}
              </span>
            ))}
          </div>
      ...
      <div className="lg:col-span-2 space-y-4 md:space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm text-center cursor-pointer group transition-all ">
            <h4 className="text-3xl font-bold text-gray-800 group-hover:text-[#EFAD44] transition-all">
          ...
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm text-center cursor-pointer group transition-all ">
            <h4 className="text-lg font-bold text-gray-800 group-hover:text-[#EFAD44] transition-all mt-2">
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 cursor-pointer group shadow-sm">
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="btn btn-sm hover:text-white hover:bg-[#244D3F] bg-gray-50 border-gray-200 text-gray-600 rounded-lg">
              <FiEdit /> Edit
            </button>
          </div>
          <p className="text-gray-600">
            Connect every{' '}
            <span className="font-bold transition-colors group-hover:text-[#EFAD44] text-[#244D3F]">
              {friend.goal} days
            </span>
          </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="grid grid-cols-3 gap-3 md:gap-4">
            {[
              {
                type: 'Call',
                icon: <LuPhoneCall className="text-xl md:text-2xl" />,
              },
              {
                type: 'Text',
                icon: <RiMessage2Line className="text-xl md:text-2xl" />,
              },
              {
                type: 'Video',
                icon: <RiVideoOnLine className="text-xl md:text-2xl" />,
              },
            ].map((item) => (
              <button
                key={item.type}
                ...
                <span className="font-bold text-xs md:text-base text-gray-700 group-hover:text-[#EFAD44] transition-colors">
                  {item.type}
                </span>
              </button>
            ))}
